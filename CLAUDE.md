...
The Skyflow Node SDK is a TypeScript client library for Skyflow's Data Privacy Vault, supporting Node.js, Deno, Bun, and Cloudflare Workers. It provides three API surfaces: **Vault** (insert, get, update, delete, query, tokenize, detokenize), **Detect** (PII deidentify/reidentify, file ops), and **Connections** (gateway invoke). Current version is 2.x.

## Critical Boundary — Generated Code

**Never edit files under `src/_generated_/`** — these are auto-generated by Fern and manual changes get overwritten on next generation. `VaultClient.initAPI()` selects the right generated client per operation. If a bug is in generated code, report it rather than patching directly.
...
Follow camelCase for acronyms: use `skyflowId`, `vaultId`, `apiKey`, `tokenUri` rather than uppercase variants. Builder/options setters follow `setReturnTokens()`, `setTokenMode()` pattern. Response fields use camelCase `skyflowId` (normalized from API snake_case). TypeScript classes: PascalCase (`InsertRequest`, `VaultController`). Functions and variables: camelCase. Constants: `UPPER_SNAKE_CASE`. Never use implicit `any`.
